...
// Common login error messages used across tests
const LOGIN_ERRORS = {
    invalidCredentials: 'Invalid email or password',

    noInstituteAssigned:
        'No institute has been assigned to this user. Please contact your administrator.',
};

// Validate returned screen after login flow
...
describe('Login Flow', () => {
    ...
    // Launch fresh app before every test
    beforeEach(async () => {
        await launchFresh();
    });

    // Ensure Detox synchronization is restored
    afterAll(async () => {
        try {
    ...
    it.only('TC01 - should show the login page layout', async () => {
        ...
        await expect(element(by.id('loginEmailInput')))
            .toBeVisible();

        await expect(element(by.id('themeToggleButton')))
            .toBeVisible();
    });
    ...
    it('TC03 - should show password input and continue button for password login', async () => {
        await choosePasswordLogin(
            USERS.oneInstituteOneRole.email,
        );

        await expect(element(by.id('loginPasswordInput')))
            .toBeVisible();

        await expect(element(by.id('loginContinueButton')))
            .toBeVisible();
    });
    ...
    it('TC04 - should support typing and scrolling on login screen', async () => {
        ...
        await element(by.id('loginEmailInput'))
            .replaceText('typing@scos.com');

        // Verify scrolling does not break layout
        await element(by.id('loginScreen'))
            .swipe('up', 'fast', 0.5);

        await element(by.id('loginScreen'))
            .swipe('down', 'fast', 0.5);

        await expect(element(by.id('loginScreen')))
            .toBeVisible();
    });
    ...
    it('TC07 - should clear login error after editing password input', async () => {
        ...
        // Correct password should clear previous error
        await element(by.id('loginPasswordInput')).tap();

        await element(by.id('loginPasswordInput'))
            .replaceText(
                USERS.oneInstituteOneRole.password,
            );
    ...
    it('TC08 - should show loader during login request', async () => {
        ...
        // Verify loader visibility during request
    ...
    it('TC09 - should show an error for a valid user with no assigned institutes', async () => {
        ...
        await expectLoginError(
            LOGIN_ERRORS.noInstituteAssigned,
        );
    });

    it('TC10 - should login a valid user with one institute and one role', async () => {
        await loginAs(
            USERS.oneInstituteOneRole,
        );

        const screen = await waitForPostLoginScreen({
            keepSynchronizationDisabled: true,
        });

        expectScreenToBeOneOf(screen, [
            'InstituteList',
            'Dashboard',
        ]);
    });
    ...
    it('TC16 - should keep logged-in session state after app restart', async () => {
        ...
        // Restart app without clearing storage
